...
#INSTRUCCIONES de uso de qmap::fitQmap y qmap::doQmap----
#fitQmap saca la funcion de transferencia que permite ajustar un vector modelado (era)
#a los quantiles de unas mediciones (anemos).Primer imput=mediciones, segundo input=modelado
#Luego, con esa ftrans podemos usar doQmap para deducir que mediciones hubiesen correspondido
#a otros datos modelados. Primer input=modelado, segudo input=ftrans. doQmap no admite NAs.
#Ejercicio interesante para entenderlo:
ftrans=fitQmapQUANT(datos_juntos$Mean,datos_juntos$uv_wind1)  #Creamos funcion de transferencia para todo el perido
...
rm(ftrans,calibrado)  #Borramos esto por que solo tienen valor "educatico"

#PRIMER EJERCICIO. Dividir datos por la mitad e intentar calibrar una mitad con la otra----
...
#SEGUNDO EJERCICIO. Dividir datos por la mitad y hacer una calibracion para cada direccion de era.
...
datos_juntos_por_dirs=list()
for (i in 1:(length(direcciones)-1)) {  #-1 porque no queremos crear un elemento para 360º
  datos_juntos_por_dirs[[i]]=data.frame()
}
datos_juntos$Dir=as.numeric(datos_juntos$Dir)
for (i in 1:nrow(datos_juntos)) { #Bucle para clasificar lineas de mitad_1 por direcciones
  (direcciones-datos_juntos$uv_dwi1[i]) %>% abs %>% which.min -> x
  ...
  if (length(datos_juntos_por_dirs[[x]])==0) {  #Si es la primera linea de la direccion x...
    datos_juntos_por_dirs[[x]]=datos_juntos[i,] #...ponemos la linea tal cual.
  }else{  #Si no es la primera linea de la direccion x, la añadimos debajo.
    datos_juntos_por_dirs[[x]][nrow(datos_juntos_por_dirs[[x]])+1,1:ncol(datos_juntos)]=datos_juntos[i,]
  }
}
direcciones=direcciones[1:(length(direcciones)-1)] #Ya no nos hace falta el 360º, solo molesta.
names(datos_juntos_por_dirs)=direcciones
...
for (i in 1:length(direcciones)) {
  lista_ftrans_mitad_1=fitQmapQUANT(datos_juntos_por_dirs[[i]]datos_juntos_por_dirs[[i]]$uv_wind1)
}
