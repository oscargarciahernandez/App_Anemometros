...
<br />
<br />
MANOS A LA OBRA. Para empezar fuerte, se me ha ocurrido que vamos a hacer funciones independientes para cada tipo de anemometro, que van a estar utilizadas en un función única. 
<br />
<br />
```{r}
...
Formatos_columna <- function(lista_datos){
  ...
  for (i in 1:numero_sensores) {
    ...
    #usamos el ya visto ifelse 
    Tabla_actualizada <- ifelse(
            letra_nombre=="a",funcion_anemo(lista_datos[[i]]),
           ifelse(letra_nombre=="p",funcion_pluv(lista_datos[[i]]),
           ifelse(letra_nombre="t",funcion_term_hig(lista_datos[[i]]),NA)))

    lista_actualizada[[i]] <- Tabla_actualizada
  }
...
Formatos_columna <- function(lista_datos){
  ...
  for (i in 1:numero_sensores) {
    ...
    #usamos el ya visto ifelse 
    Tabla_actualizada <- ifelse(
            letra_nombre=="a",possibly(funcion_anemo(lista_datos[[i]]),otherwise = NA),
           ifelse(letra_nombre=="p",possibly(funcion_pluv(lista_datos[[i]]),otherwise = NA),
           ifelse(letra_nombre="t",possibly(funcion_term_hig(lista_datos[[i]]),otherwise = NA),NA)))
