...
WEIBULL_PLOT<- function(DATOS_INPUT, ACUMULATED= TRUE, DISTRIBUTION= TRUE){
...
ACUM_WEIBULL<- ggplot() + 
  ...
  geom_histogram(data=DATOS_weibull,
                 ...
                 fill="blue")+
  geom_line(aes(x=seq(0,10,by=bind_anch),
                y=cdweibull(DATOS_weibull$ERAWS, bind_anch)*shape_factor_acum), col="darkred")+
  geom_line(aes(x=seq(0,10,by=bind_anch),
                y=cdweibull(DATOS_weibull$WS_N, bind_anch)*shape_factor_acum), col="darkblue")+
  ylab("")+
  xlab("Wind Speed [m/s]")+
...
DATOS_weibull1<- WEIBULL_PLOT(PUNTO_MC)
...
DATOS_weibull2<- WEIBULL_PLOT(GRUPO_TEST)
...
A continuación realizamos un ejercicio aplicando el quantile mapping por direcciones... de esta manera se pretende crear 16 funciones de transferencia que nos permitan obtener una predicción más ajustada. 
...
#JUNTAMOS DE NUEVO EN FORMATO TABLA Y LA ORDENAMOS CRONOLÓGICAMENTE. A CONTINUACIÓN PROBAMOS
MERGE_TEST<- LISTA_TEST %>% bind_rows() %>% .[order(.$Date),]

```

```{r eval=TRUE}
DATOS_weibull3<-  WEIBULL_PLOT(MERGE_TEST)
...
Se puede observar como no existe una mejora notable respecto al caso anterior en el que se creaba una unica función de transferencia. Por lo tanto pasamos a evaluar otras manera de agrupar. De hecho se puede apreciar un empeoramiento en la correlacion. 
...
```{r eval=TRUE}
DATOS_weibull8<- WEIBULL_PLOT(MERGE_TEST)
