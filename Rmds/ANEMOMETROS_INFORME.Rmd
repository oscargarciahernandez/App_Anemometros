...
DATOS_PLOT<- DATOS_JUNTOS_LISTA[[which.min(Tabla_dist)]] %>%
  mutate(ERA_binDir= cut(ERAWD ,
                         breaks = c(0,seq(11.5,349.5,22.5),360, 361), 
                         labels = c(0,seq(11.5,349.5,22.5),0) %>% as.factor()))
...
conditionalPanel('input.SELECTOR=="Cross-correlation"',
   ...
   plotData <- reactive({
     DATOS_PLOT %>% .[complete.cases(.),] %>%  mutate(ERAWS=SMA(ERAWS, input$SMA_value),
  ...
  mutate(ANEM_selected= ifelse(rep(input$Interpolation=="linear",nrow(.)),DATOS_PLOT$WS_l,
  ...
  mutate(GUST= ifelse(!input$GUST,ANEM_selected,
                      ifelse(rep(input$Interpolation=="linear",nrow(.)),WSMAX_l,
                             ifelse(rep(input$Interpolation=="spline",nrow(.)), WSMAX_sp,
                                    ifelse(rep(input$Interpolation=="stine",nrow(.)), WSMAX_st,
                                           ifelse(rep(input$Interpolation=="Nearest",nrow(.)), WSMAX_N,NA))))))
