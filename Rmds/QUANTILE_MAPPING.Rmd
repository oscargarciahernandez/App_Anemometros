...
#PRIMERO DEFINIMOS FUNCIONES BÁSICAS
...
# ANALISIS DE LOS PUNTOS MAS CERCANOS ERA5 CON 1 AÑO DE DATOS
...
# add a legend
legend(lpos+ 1.9,lpos+0.5,legend=paste(round(Tabla_dist[rev(order((Tabla_dist)))]/1000, 2), 'km')[seq(1,length(Tabla_dist), length.out = 10)],
       pch=19,col=viridis(length(rev(order((Tabla_dist)))))[seq(1,length(Tabla_dist), length.out = 10)])
...
```{r  fig.height = 10, fig.width = 10, fig.align = "center", eval=TRUE}
library(elevatr)
library(rasterVis)
incr<- 0.02
...
NOMBRE_RASTER<- paste0(PATH_MAPS, '/map_raster_',
                       paste(ul, collapse = ''),
                       paste(lr, collapse = ''), '.RDS')
...
for(i in 1:length(INDICES_ERROR)){
  ...
  #AÑADO ESTE IF PARA REPRESENTAR EL MEJOR VALOR DE CORRELACION DE ACUERDO A LOS COLORES EMPLEADOS
...
# CORRECTED WITH UNVARIANT SURFACE ROUGHNESS
...
k<- 1
TABLA_ACCURACY_C<- data.frame(matrix(ncol = 8))
for (i in rev(order((Tabla_dist)))) {
  DATOS_PLOT<- DATOS_JUNTOS_LISTA[[i]] %>%
  mutate(ERA_binDir= cut(ERAWD ,
                         breaks =c(0,seq(22.5,337.5,22.5),360, 361), 
...
if(k==1){
   taylor.diagram(as.vector(DATOS_PLOT$WS_N), 
                 as.vector(DATOS_PLOT$ERAWS_CORRECTED), col = viridis(length(rev(order((Tabla_dist)))))[k])
   taylor.diagram(as.vector(DATOS_PLOT$WS_N), 
                 as.vector(DATOS_PLOT$WS_N),add = TRUE)
}
  
 taylor.diagram(as.vector(DATOS_PLOT$WS_N), 
                 as.vector(DATOS_PLOT$ERAWS_CORRECTED),add = TRUE, col = viridis(length(rev(order((Tabla_dist)))))[k], pcex = 1, alpha=0.5)
  


TABLA_ACCURACY_C[k, ]<- c(DATOS_PLOT$ERAlon %>% unique(), 
                        DATOS_PLOT$ERAlat %>% unique(),
                        accuracy(as.vector(DATOS_PLOT$WS_N),
                                 as.vector(DATOS_PLOT$ERAWS_CORRECTED)),
                        cor(as.vector(DATOS_PLOT$WS_N),
                                 as.vector(DATOS_PLOT$ERAWS_CORRECTED), use = 'complete.obs'))
 k<-k+1

}
lpos<-2*sd(DATOS_PLOT$WS_N)
# add a legend
legend(lpos+ 9,lpos+5,legend=paste(round(Tabla_dist[rev(order((Tabla_dist)))]/1000, 2), 'km')[seq(1,length(Tabla_dist), length.out = 10)],
       pch=19,col=viridis(length(rev(order((Tabla_dist)))))[seq(1,length(Tabla_dist), length.out = 10)])


rownames(TABLA_ACCURACY_C)<- paste(round(Tabla_dist[rev(order((Tabla_dist)))]/1000, 2), 'km')
colnames(TABLA_ACCURACY_C)<- c( 'LON','LAT', 'ME','RMSE','MAE','MPE', 'MAPE', 'CORR')
kable(TABLA_ACCURACY_C)
...
n=max(TABLA_ACCURACY_C$LAT)+ incr  
s=min(TABLA_ACCURACY_C$LAT)- incr
e=max(TABLA_ACCURACY_C$LON)+ incr
w=min(TABLA_ACCURACY_C$LON)- incr
...
INDICES_ERROR<- colnames(TABLA_ACCURACY_C)[c(3,4,5,8)] 
for(i in 1:length(INDICES_ERROR)){
  
  nombre_ind<- INDICES_ERROR[i]
  columna<- TABLA_ACCURACY_C[, which(colnames(TABLA_ACCURACY_C)==nombre_ind)]
  ...
  if(nombre_ind=='CORR'){
      ...
      p2 <- spplot(SpatialPointsDataFrame(TABLA_ACCURACY_C[order(columna),][,c('LON', 'LAT')],
                                      data = data.frame(IND=columna[order(columna)] %>% 
                                                          round(4)%>% as.factor())),
               pch=20, colour='NA', 
               cex=3,zcol= c('IND'),
               col.regions= viridis(nrow(TABLA_ACCURACY_C))[1:nrow(TABLA_ACCURACY_C)])
  }else{
     p2 <- spplot(SpatialPointsDataFrame(TABLA_ACCURACY_C[order(columna),][,c('LON', 'LAT')],
                                      data = data.frame(IND=columna[order(columna)] %>% 
                                                          round(4)%>% as.factor())),
               pch=20, colour='NA', 
               cex=3,zcol= c('IND'),
               col.regions= rev(viridis(nrow(TABLA_ACCURACY_C))[1:nrow(TABLA_ACCURACY_C)]))
...
```{r eval=TRUE, fig.height=6, fig.width=6, eval=FALSE}
# DIAGRAMA DE TAYLOR FILTRADO POR DIRECCIONES

#La siguiente gráfica no se si me gusta... porque el filtrado por direcciones lo he hecho haciendo grupos de 3. Tenemos 16 direcciones en total y hemos dividio en 8 direcciones. Es decir, la direccion 292.5º cuenta el grupo de W y NW. Se podria mejorar haciendo 8 grupos de 2 direcciones o 16 grupos de 1. 
...
for (j in 1:length(lista_dir)) {
  TABLA_ACCURACY_C<- data.frame(matrix(ncol = 10))
...
TABLA_ACCURACY_C[k, ]<- c(DATOS_PLOT$ERAlon %>% unique() %>% round(2), 
...
colnames(TABLA_ACCURACY_C)<- c( 'LON','LAT','DISTANCIA','DIRECCION',
                              'ME','RMSE','MAE','MPE', 'MAPE', 'CORR')

  LISTA_ACCURACY[[j]]<- TABLA_ACCURACY_C  
...
## CORRECTED USING ERA5 SURFACE ROUGHNESS
...
k<- 1
TABLA_ACCURACY_C2<- data.frame(matrix(ncol = 8))
for (i in rev(order((Tabla_dist)))) {
  DATOS_PLOT<- DATOS_JUNTOS_LISTA[[i]] %>%
  mutate(ERA_binDir= cut(ERAWD ,
                         breaks =c(0,seq(22.5,337.5,22.5),360, 361), 
...
if(k==1){
   taylor.diagram(as.vector(DATOS_PLOT$WS_N), 
                 as.vector(DATOS_PLOT$ERAWS_CORRECTED_ZO1), col = viridis(length(rev(order((Tabla_dist)))))[k])
   taylor.diagram(as.vector(DATOS_PLOT$WS_N), 
                 as.vector(DATOS_PLOT$WS_N),add = TRUE)
}
  
 taylor.diagram(as.vector(DATOS_PLOT$WS_N), 
                 as.vector(DATOS_PLOT$ERAWS_CORRECTED_ZO1),add = TRUE, col = viridis(length(rev(order((Tabla_dist)))))[k], pcex = 1, alpha=0.5)
  


TABLA_ACCURACY_C2[k, ]<- c(DATOS_PLOT$ERAlon %>% unique(), 
                        DATOS_PLOT$ERAlat %>% unique(),
                        accuracy(as.vector(DATOS_PLOT$WS_N),
                                 as.vector(DATOS_PLOT$ERAWS_CORRECTED_ZO1)),
                        cor(as.vector(DATOS_PLOT$WS_N),
                                 as.vector(DATOS_PLOT$ERAWS_CORRECTED_ZO1), use = 'complete.obs'))
 k<-k+1
...
rownames(TABLA_ACCURACY_C2)<- paste(round(Tabla_dist[rev(order((Tabla_dist)))]/1000, 2), 'km')
colnames(TABLA_ACCURACY_C2)<- c( 'LON','LAT', 'ME','RMSE','MAE','MPE', 'MAPE', 'CORR')
kable(TABLA_ACCURACY_C2)
...
n=max(TABLA_ACCURACY_C2$LAT)+ incr  
s=min(TABLA_ACCURACY_C2$LAT)- incr
e=max(TABLA_ACCURACY_C2$LON)+ incr
w=min(TABLA_ACCURACY_C2$LON)- incr
...
INDICES_ERROR<- colnames(TABLA_ACCURACY_C2)[c(3,4,5,8)] 
for(i in 1:length(INDICES_ERROR)){
  
  nombre_ind<- INDICES_ERROR[i]
  columna<- TABLA_ACCURACY_C2[, which(colnames(TABLA_ACCURACY_C2)==nombre_ind)]
  ...
  if(nombre_ind=='CORR'){
      ...
      p2 <- spplot(SpatialPointsDataFrame(TABLA_ACCURACY_C2[order(columna),][,c('LON', 'LAT')],
                                      data = data.frame(IND=columna[order(columna)] %>% 
                                                          round(4)%>% as.factor())),
               pch=20, colour='NA', 
               cex=3,zcol= c('IND'),
               col.regions= viridis(nrow(TABLA_ACCURACY_C2))[1:nrow(TABLA_ACCURACY_C2)])
  }else{
     p2 <- spplot(SpatialPointsDataFrame(TABLA_ACCURACY_C2[order(columna),][,c('LON', 'LAT')],
                                      data = data.frame(IND=columna[order(columna)] %>% 
                                                          round(4)%>% as.factor())),
               pch=20, colour='NA', 
               cex=3,zcol= c('IND'),
               col.regions= rev(viridis(nrow(TABLA_ACCURACY_C2))[1:nrow(TABLA_ACCURACY_C2)]))
...
LISTA_ACCURACY<- list()
for (j in 1:length(lista_dir)) {
  TABLA_ACCURACY_C2<- data.frame(matrix(ncol = 10))
...
TABLA_ACCURACY_C2[k, ]<- c(DATOS_PLOT$ERAlon %>% unique() %>% round(2), 
                        DATOS_PLOT$ERAlat %>% unique()%>% round(2),
                        Tabla_dist[i]%>% round(2),
                        lista_dir[[j]] %>% paste(collapse = '-'),
                        accuracy(as.vector(DATOS_PLOT$WS_N)%>% round(2),
...
colnames(TABLA_ACCURACY_C2)<- c( 'LON','LAT','DISTANCIA','DIRECCION',
                              'ME','RMSE','MAE','MPE', 'MAPE', 'CORR')

  LISTA_ACCURACY[[j]]<- TABLA_ACCURACY_C2  
...
```{r eval=TRUE}
TABLA_ACCURACY$id<- 'UNCORRECTED'
TABLA_ACCURACY_C$id<- 'CORRECTED_UNVARIANT_ZO'
TABLA_ACCURACY_C2$id<- 'CORRECTED_VARIANT_ZO'

TABLA_ACCURACY_TOTAL<- bind_rows(list(TABLA_ACCURACY,
                                      TABLA_ACCURACY_C, 
                                      TABLA_ACCURACY_C2))

ggplot()+
  geom_boxplot(data = TABLA_ACCURACY_TOTAL, aes(y= ME, x= id))+
  theme_light()

ggplot()+
  geom_boxplot(data = TABLA_ACCURACY_TOTAL, aes(y= RMSE, x= id))+
  theme_light()

ggplot()+
  geom_boxplot(data = TABLA_ACCURACY_TOTAL, aes(y= MAE, x= id))+
  theme_light()

ggplot()+
  geom_boxplot(data = TABLA_ACCURACY_TOTAL, aes(y= CORR, x= id))+
  theme_light()
```
